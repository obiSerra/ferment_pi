...
class Screen():
    ...
    def new_screen(self, bg_color=1):
        self.image = Image.new(
            '1', (self.disp.width, self.disp.height), bg_color)

    def render(self):
        self.disp.ShowImage(self.disp.getbuffer(self.image))
        time.sleep(1)

    def draw_border(self, top=True, bottom=True, left=True, right=True,
                    fill=0):
    ...
    def draw_text(self, text, font_size=13, position=(0, 0), font='Font.ttf',
                  bg_color=1, fill=0):
        if not self.image:
